...
class Queries2 {
	...
	public static VitalGraphQuery connectionsQueyGraph(List<VitalSegment> segments, String inputURI, Integer depth, int offset, int limit, List<Class<? extends VITAL_Edge>> forwardEdgeTypes, List<Class<? extends VITAL_Edge>> reverseEdgeTypes) {
		...
		//both
		List<URIProperty> fClasses = [];
		for(Class<? extends VITAL_Edge> fC : forwardEdgeTypes) {
			fClasses.add(URIProperty.withString(VitalSigns.get().getClassesRegistry().getClassURI(fC)));
		}
		
		List<URIProperty> rClasses = [];
		for(Class<? extends VITAL_Edge> rC : reverseEdgeTypes) {
			rClasses.add(URIProperty.withString(VitalSigns.get().getClassesRegistry().getClassURI(rC)));
		}
		
		VitalGraphQueryPropertyCriterion forwardCrit = new VITAL_Edge_PropertiesHelper().vitaltype.oneOf(fClasses)
		VitalGraphQueryPropertyCriterion reverseCrit = new VITAL_Edge_PropertiesHelper().vitaltype.oneOf(rClasses)
